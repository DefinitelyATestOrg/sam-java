...
import software.elborai.api.services.async.EntityServiceAsync
...
class EntityListPageAsync private constructor(private val entitiesService: EntityServiceAsync, private val params: EntityListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<EntityListParams> {
      ...
      return Optional.of(EntityListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<EntityListPageAsync>> {
      return getNextPageParams().map {
        entitiesService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: EntityListPageAsync, ) {

        fun forEach(action: Predicate<Entity>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<EntityListPageAsync>>.forEach(action: (Entity) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
