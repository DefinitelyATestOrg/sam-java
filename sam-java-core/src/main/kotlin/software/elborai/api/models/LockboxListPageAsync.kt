...
class LockboxListPageAsync private constructor(private val lockboxesService: LockboxServiceAsync, private val params: LockboxListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<LockboxListParams> {
      ...
      return Optional.of(LockboxListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<LockboxListPageAsync>> {
      return getNextPageParams().map {
        lockboxesService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: LockboxListPageAsync, ) {

        fun forEach(action: Predicate<Lockbox>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<LockboxListPageAsync>>.forEach(action: (Lockbox) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
