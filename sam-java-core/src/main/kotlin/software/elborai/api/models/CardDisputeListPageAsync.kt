...
import software.elborai.api.services.async.CardDisputeServiceAsync
...
class CardDisputeListPageAsync private constructor(private val cardDisputesService: CardDisputeServiceAsync, private val params: CardDisputeListParams, private val response: Response, ) {
    ...
    fun getNextPage(): CompletableFuture<Optional<CardDisputeListPageAsync>> {
      return getNextPageParams().map {
        cardDisputesService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: CardDisputeListPageAsync, ) {

        fun forEach(action: Predicate<CardDispute>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<CardDisputeListPageAsync>>.forEach(action: (CardDispute) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
