...
import software.elborai.api.services.async.DocumentServiceAsync
...
class DocumentListPageAsync private constructor(private val documentsService: DocumentServiceAsync, private val params: DocumentListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<DocumentListParams> {
      ...
      return Optional.of(DocumentListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<DocumentListPageAsync>> {
      return getNextPageParams().map {
        documentsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: DocumentListPageAsync, ) {

        fun forEach(action: Predicate<Document>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<DocumentListPageAsync>>.forEach(action: (Document) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
