...
import software.elborai.api.services.async.WireTransferServiceAsync
...
class WireTransferListPageAsync private constructor(private val wireTransfersService: WireTransferServiceAsync, private val params: WireTransferListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<WireTransferListParams> {
      ...
      return Optional.of(WireTransferListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }
    ...
    class AutoPager constructor(private val firstPage: WireTransferListPageAsync, ) {

        fun forEach(action: Predicate<WireTransfer>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<WireTransferListPageAsync>>.forEach(action: (WireTransfer) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
