...
import software.elborai.api.services.async.ProgramServiceAsync
...
class ProgramListPageAsync private constructor(private val programsService: ProgramServiceAsync, private val params: ProgramListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<ProgramListParams> {
      ...
      return Optional.of(ProgramListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<ProgramListPageAsync>> {
      return getNextPageParams().map {
        programsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: ProgramListPageAsync, ) {

        fun forEach(action: Predicate<Program>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<ProgramListPageAsync>>.forEach(action: (Program) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
