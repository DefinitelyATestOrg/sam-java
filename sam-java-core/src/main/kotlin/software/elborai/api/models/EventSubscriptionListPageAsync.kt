...
import software.elborai.api.services.async.EventSubscriptionServiceAsync
...
class EventSubscriptionListPageAsync private constructor(private val eventSubscriptionsService: EventSubscriptionServiceAsync, private val params: EventSubscriptionListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<EventSubscriptionListParams> {
      ...
      return Optional.of(EventSubscriptionListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }
    ...
    class AutoPager constructor(private val firstPage: EventSubscriptionListPageAsync, ) {

        fun forEach(action: Predicate<EventSubscription>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<EventSubscriptionListPageAsync>>.forEach(action: (EventSubscription) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
