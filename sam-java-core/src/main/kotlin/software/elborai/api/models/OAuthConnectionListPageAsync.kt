...
import software.elborai.api.services.async.OAuthConnectionServiceAsync
...
class OAuthConnectionListPageAsync private constructor(private val oauthConnectionsService: OAuthConnectionServiceAsync, private val params: OAuthConnectionListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<OAuthConnectionListParams> {
      ...
      return Optional.of(OAuthConnectionListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }
    ...
    class AutoPager constructor(private val firstPage: OAuthConnectionListPageAsync, ) {

        fun forEach(action: Predicate<OAuthConnection>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<OAuthConnectionListPageAsync>>.forEach(action: (OAuthConnection) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
