...
class IntrafiAccountEnrollmentListPageAsync private constructor(private val accountEnrollmentsService: AccountEnrollmentServiceAsync, private val params: IntrafiAccountEnrollmentListParams, private val response: Response, ) {
    ...
    override fun toString() = "IntrafiAccountEnrollmentListPageAsync{accountEnrollmentsService=$accountEnrollmentsService, params=$params, response=$response}"
    ...
    fun getNextPage(): CompletableFuture<Optional<IntrafiAccountEnrollmentListPageAsync>> {
      return getNextPageParams().map {
        accountEnrollmentsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    companion object {
        ...
        @JvmStatic
        fun of(accountEnrollmentsService: AccountEnrollmentServiceAsync, params: IntrafiAccountEnrollmentListParams, response: Response) = IntrafiAccountEnrollmentListPageAsync(
            accountEnrollmentsService,
            params,
            response,
        )
    ...
    class AutoPager constructor(private val firstPage: IntrafiAccountEnrollmentListPageAsync, ) {

        fun forEach(action: Predicate<IntrafiAccountEnrollment>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<IntrafiAccountEnrollmentListPageAsync>>.forEach(action: (IntrafiAccountEnrollment) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
