...
import software.elborai.api.services.async.RoutingNumberServiceAsync
...
class RoutingNumberListPageAsync private constructor(private val routingNumbersService: RoutingNumberServiceAsync, private val params: RoutingNumberListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<RoutingNumberListParams> {
      ...
      return Optional.of(RoutingNumberListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<RoutingNumberListPageAsync>> {
      return getNextPageParams().map {
        routingNumbersService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    @NoAutoDetect
    class Response constructor(private val data: JsonField<List<RoutingNumber>>, private val nextCursor: JsonField<String>, private val additionalProperties: Map<String, JsonValue>, ) {
    ...
    class AutoPager constructor(private val firstPage: RoutingNumberListPageAsync, ) {

        fun forEach(action: Predicate<RoutingNumber>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<RoutingNumberListPageAsync>>.forEach(action: (RoutingNumber) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
