...
import software.elborai.api.services.async.DigitalWalletTokenServiceAsync
...
class DigitalWalletTokenListPageAsync private constructor(private val digitalWalletTokensService: DigitalWalletTokenServiceAsync, private val params: DigitalWalletTokenListParams, private val response: Response, ) {
    ...
    fun getNextPage(): CompletableFuture<Optional<DigitalWalletTokenListPageAsync>> {
      return getNextPageParams().map {
        digitalWalletTokensService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: DigitalWalletTokenListPageAsync, ) {

        fun forEach(action: Predicate<DigitalWalletToken>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<DigitalWalletTokenListPageAsync>>.forEach(action: (DigitalWalletToken) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
