...
import software.elborai.api.services.async.AccountNumberServiceAsync
...
class AccountNumberListPageAsync private constructor(private val accountNumbersService: AccountNumberServiceAsync, private val params: AccountNumberListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<AccountNumberListParams> {
      ...
      return Optional.of(AccountNumberListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }
    ...
    class AutoPager constructor(private val firstPage: AccountNumberListPageAsync, ) {

        fun forEach(action: Predicate<AccountNumber>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<AccountNumberListPageAsync>>.forEach(action: (AccountNumber) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
