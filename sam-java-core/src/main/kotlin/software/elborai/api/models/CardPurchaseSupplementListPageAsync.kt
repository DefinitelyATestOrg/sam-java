...
import software.elborai.api.services.async.CardPurchaseSupplementServiceAsync
...
class CardPurchaseSupplementListPageAsync private constructor(private val cardPurchaseSupplementsService: CardPurchaseSupplementServiceAsync, private val params: CardPurchaseSupplementListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<CardPurchaseSupplementListParams> {
      ...
      return Optional.of(CardPurchaseSupplementListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<CardPurchaseSupplementListPageAsync>> {
      return getNextPageParams().map {
        cardPurchaseSupplementsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class Response constructor(private val data: JsonField<List<CardPurchaseSupplement>>, private val nextCursor: JsonField<String>, private val additionalProperties: Map<String, JsonValue>, ) {
        ...
        @JsonProperty("data")
        fun _data(): Optional<JsonField<List<CardPurchaseSupplement>>> = Optional.ofNullable(data)
    ...
    class AutoPager constructor(private val firstPage: CardPurchaseSupplementListPageAsync, ) {

        fun forEach(action: Predicate<CardPurchaseSupplement>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<CardPurchaseSupplementListPageAsync>>.forEach(action: (CardPurchaseSupplement) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
