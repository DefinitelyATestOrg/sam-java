...
import software.elborai.api.services.async.CardServiceAsync
...
class CardListPageAsync private constructor(private val cardsService: CardServiceAsync, private val params: CardListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<CardListParams> {
      ...
      return Optional.of(CardListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<CardListPageAsync>> {
      return getNextPageParams().map {
        cardsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: CardListPageAsync, ) {

        fun forEach(action: Predicate<Card>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<CardListPageAsync>>.forEach(action: (Card) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
