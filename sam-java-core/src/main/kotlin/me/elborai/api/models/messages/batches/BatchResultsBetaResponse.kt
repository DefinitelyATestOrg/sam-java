...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        /**
         * Returns an immutable instance of [BatchResultsBetaResponse].
         *
            ...
            override fun ObjectCodec.deserialize(node: JsonNode): Result {
                ...
                when (type) {
                    "succeeded" -> {
                        return Result(
                            betaSucceeded =
                                deserialize(node, jacksonTypeRef<BetaSucceededResult>()),
                            _json = json,
                        )
                    }
                    "errored" -> {
                        return Result(
                            betaErrored = deserialize(node, jacksonTypeRef<BetaErroredResult>()),
                            _json = json,
                        )
                    }
                    "canceled" -> {
                        return Result(
                            betaCanceled = deserialize(node, jacksonTypeRef<BetaCanceledResult>()),
                            _json = json,
                        )
                    }
                    "expired" -> {
                        return Result(
                            betaExpired = deserialize(node, jacksonTypeRef<BetaExpiredResult>()),
                            _json = json,
                        )
                    }
                }
                        ...
                        override fun ObjectCodec.deserialize(node: JsonNode): InnerError {
                            ...
                            when (type) {
                                ...
                                "billing_error" -> {
                                    return InnerError(
                                        betaBilling =
                                            deserialize(node, jacksonTypeRef<BetaBillingError>()),
                                        _json = json,
                                    )
                                }
                                "permission_error" -> {
                                ...
                                "not_found_error" -> {
                                    return InnerError(
                                        betaNotFound =
                                            deserialize(node, jacksonTypeRef<BetaNotFoundError>()),
                                        _json = json,
                                    )
                                }
                                "rate_limit_error" -> {
                                    return InnerError(
                                        betaRateLimit =
                                            deserialize(node, jacksonTypeRef<BetaRateLimitError>()),
                                        _json = json,
                                    )
                                }
                                "timeout_error" -> {
                                ...
                                "api_error" -> {
                                    return InnerError(
                                        betaApi = deserialize(node, jacksonTypeRef<BetaApiError>()),
                                        _json = json,
                                    )
                                }
                                "overloaded_error" -> {
